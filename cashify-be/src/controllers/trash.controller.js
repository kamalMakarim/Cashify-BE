...
exports.create = async (req, res) => {
  if (!req.body.trashType) {
    return res.status(400).json({ message: "Trash type is required" });
  }
  if (!req.body.collecter_id) {
    return res.status(400).json({ message: "Collecter ID is required" });
  }
  try {
    const { trashType, collecter_id } = req.body;
    const trash = new Trash({ trashType, collected_at: collecter_id });
    await trash.save();
    res.json({ success: true, message: "Trash created successfully" });
  } catch (error) {
...
exports.claim = async (req, res) => {
  if (!req.body.trashId) {
    return res.status(400).json({ message: "Trash ID is required" });
  }
  if (!req.user._id) {
  ...
  try {
    ...
    if (!trash) {
      return res.status(400).json({ message: "Trash not found" });
    }
    await trash.save();
